...
class TNM_Trust_REST {
	...
	public static function register_routes() {
		...
		register_rest_route(
			self::NS,
			'/offers/checkout/start',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( __CLASS__, 'start_offer_checkout' ),
				'permission_callback' => array( __CLASS__, 'permission_logged_in' ),
			)
		);

		// Mobile/native-app-friendly alternative to /offers/checkout/start —
		// creates a real WC order and returns the native Stripe PaymentSheet
		// payload instead of relying on a browser cart session. Use this from
		// the app; use the /start route only for browser/web checkouts.
		register_rest_route(
			self::NS,
			'/offers/checkout/order',
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( __CLASS__, 'create_offer_checkout_order' ),
				'permission_callback' => array( __CLASS__, 'permission_logged_in' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::CREATABLE,
					'callback'            => array( __CLASS__, 'add_to_bundle_builder' ),
					'permission_callback' => array( __CLASS__, 'permission_logged_in' ),
				),
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( __CLASS__, 'get_bundle_builder' ),
					'permission_callback' => array( __CLASS__, 'permission_logged_in' ),
				),
			)
		);
	...
	public static function create_offer( WP_REST_Request $request ) {
		$args = array(
			...
			'offer_price' => $request->get_param( 'offer_price' ),
		);

		return self::respond( TNM_Trust_Offers::create_offer( get_current_user_id(), $args ) );
	}

	public static function list_offers( WP_REST_Request $request ) {
		$filters = array( 'status' => sanitize_key( (string) $request->get_param( 'status' ) ) );
		return self::respond( TNM_Trust_Offers::list_offers_for_user( get_current_user_id(), $filters ) );
	}
	...
	public static function update_offer( WP_REST_Request $request ) {
		...
		$args = array(
			...
			'counter_price' => $request->get_param( 'counter_price' ),
		);

		return self::respond( TNM_Trust_Offers::update_offer( $id, $args, get_current_user_id() ) );
	}

	public static function start_offer_checkout( WP_REST_Request $request ) {
		$token  = (string) $request->get_param( 'token' );
		$result = TNM_Trust_Offers::start_offer_checkout( $token );
	...
	public static function create_offer_checkout_order( WP_REST_Request $request ) {
		$token  = (string) $request->get_param( 'token' );
		$result = TNM_Trust_Offers::create_direct_order_for_offer( $token, get_current_user_id() );

		return self::respond( $result );
	}

	public static function add_to_bundle_builder( WP_REST_Request $request ) {
		$product_id = absint( $request->get_param( 'product_id' ) );
		return self::respond( TNM_Trust_Offers::add_to_bundle_builder( $product_id ) );
	}

	public static function get_bundle_builder( WP_REST_Request $request ) {
		return self::respond( TNM_Trust_Offers::get_bundle_builder() );
	}

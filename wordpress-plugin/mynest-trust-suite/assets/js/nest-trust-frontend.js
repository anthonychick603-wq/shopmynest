...
( function () {
	...
	/* ------------------------------------------------------------------
	 * Make an offer / bundle builder widgets.
	 * ------------------------------------------------------------------ */
	document.addEventListener( 'click', function ( event ) {
		var offerBtn = event.target.closest( '.tnm-trust-make-offer-btn' );
		if ( offerBtn ) {
			var widget = offerBtn.closest( '.tnm-trust-make-offer-widget' );
			var modal = widget.querySelector( '.tnm-trust-make-offer-modal' );
			modal.hidden = ! modal.hidden;
			return;
		}

		var submitOfferBtn = event.target.closest( '.tnm-trust-submit-offer-btn' );
		if ( submitOfferBtn ) {
			var offerWidget = submitOfferBtn.closest( '.tnm-trust-make-offer-widget' );
			var productId = parseInt( offerWidget.getAttribute( 'data-product-id' ), 10 );
			var priceInput = offerWidget.querySelector( '.tnm-trust-offer-price-input' );
			var msgEl = offerWidget.querySelector( '.tnm-trust-form-message' );
			var price = parseFloat( priceInput.value );

			if ( ! price || price <= 0 ) {
				msgEl.textContent = 'Please enter a valid offer price.';
				msgEl.className = 'tnm-trust-form-message is-error';
				return;
			}

			nestTrustFetch( 'offers', {
				method: 'POST',
				body: JSON.stringify( { type: 'single', product_ids: [ productId ], offer_price: price } ),
			} ).then( function () {
				msgEl.textContent = 'Offer sent to the seller!';
				msgEl.className = 'tnm-trust-form-message is-success';
			} ).catch( function ( err ) {
				msgEl.textContent = err.message || CONFIG.i18n.error;
				msgEl.className = 'tnm-trust-form-message is-error';
			} );
			return;
		}

		var addToBundleBtn = event.target.closest( '.tnm-trust-add-to-bundle-btn' );
		if ( addToBundleBtn ) {
			var bundleWidget = addToBundleBtn.closest( '.tnm-trust-make-offer-widget' );
			var bundleProductId = parseInt( bundleWidget.getAttribute( 'data-product-id' ), 10 );

			nestTrustFetch( 'bundle-builder', {
				method: 'POST',
				body: JSON.stringify( { product_id: bundleProductId } ),
			} ).then( function ( data ) {
				var bar = document.getElementById( 'tnm-trust-bundle-bar' );
				if ( bar ) {
					bar.hidden = false;
					bar.innerHTML = '<span>' + data.product_ids.length + ' item(s) in your bundle from this seller</span>' +
						'<button type="button" class="tnm-trust-btn tnm-trust-make-bundle-offer-btn" data-seller-id="' + data.seller_id + '">Make Bundle Offer</button>';
				}
			} ).catch( function ( err ) {
				alert( err.message || CONFIG.i18n.error ); // eslint-disable-line no-alert
			} );
	...
	/* ------------------------------------------------------------------
	 * Accepted-offer checkout flow.
	 * ------------------------------------------------------------------ */
	var offerCheckoutRoot = document.getElementById( 'tnm-trust-offer-checkout' );
	if ( offerCheckoutRoot ) {
		var token = offerCheckoutRoot.getAttribute( 'data-token' );

		nestTrustFetch( 'offers/checkout/start', {
			method: 'POST',
			body: JSON.stringify( { token: token } ),
		} ).then( function ( data ) {
			offerCheckoutRoot.innerHTML = '<p>Your accepted-offer price has been applied. Redirecting to checkout…</p>';
			window.location.href = data.checkout_url;
		} ).catch( function ( err ) {
			offerCheckoutRoot.innerHTML = '<p class="tnm-trust-notice">' + ( err.message || CONFIG.i18n.error ) + '</p>';
		} );
	}
}() );

...
final class MNU_Social_Frontend {
	...
	public static function sc_messages( $atts = array() ): string {
		...
		<div class="mnu-messages" data-mnu-messages data-preselect="<?php echo esc_attr( $preselected ); ?>" data-product-context="<?php echo esc_attr( $product_ctx ); ?>">
			<div class="mnu-messages__sidebar" data-mnu-conversations>
			...
			<div class="mnu-messages__thread" data-mnu-thread>
				<div class="mnu-messages__empty" data-mnu-thread-empty>
					<p class="mnu-muted">Pick a conversation, or open a shop and click <em>Message shop</em> to start a new one.</p>
				</div>
			</div>
		</div>
		<?php
	...
	private static function inline_css(): string {
		...
		.mnu-messages{display:grid;grid-template-columns:280px 1fr;gap:1rem;min-height:70vh}
		@media(max-width:720px){.mnu-messages{grid-template-columns:1fr}}
		.mnu-messages__sidebar,.mnu-messages__thread{background:#fffdf7;border:1px solid #ecdfcd;border-radius:12px;padding:1rem;overflow-y:auto}
		.mnu-conv-row{display:flex;gap:.6rem;align-items:center;padding:.6rem;border-radius:8px;cursor:pointer}
		.mnu-conv-row:hover,.mnu-conv-row.is-active{background:#f4ecdc}
		.mnu-conv-row.is-unread .mnu-conv-row__name{font-weight:700}
		.mnu-conv-row img{width:40px;height:40px;border-radius:50%;object-fit:cover;flex-shrink:0;background:#f4ecdc}
		.mnu-conv-row__body{flex:1;min-width:0}
		.mnu-conv-row__name{font-size:.9rem}
		.mnu-conv-row__preview{font-size:.8rem;color:#7a6b57;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
		.mnu-thread__head{display:flex;gap:.75rem;align-items:center;padding-bottom:.75rem;border-bottom:1px solid #ecdfcd;margin-bottom:.75rem}
		.mnu-thread__head img{width:40px;height:40px;border-radius:50%;object-fit:cover}
		.mnu-thread__msgs{display:flex;flex-direction:column;gap:.5rem;padding:.5rem 0 1rem}
		.mnu-msg{max-width:75%;padding:.5rem .75rem;border-radius:12px;font-size:.95rem;white-space:pre-wrap;word-wrap:break-word}
		.mnu-msg.mine{align-self:flex-end;background:#245f4b;color:#fff;border-bottom-right-radius:4px}
		.mnu-msg.theirs{align-self:flex-start;background:#f4ecdc;color:#2b2016;border-bottom-left-radius:4px}
		.mnu-msg-form{display:flex;gap:.5rem;padding-top:.75rem;border-top:1px solid #ecdfcd}
		.mnu-msg-form textarea{flex:1;resize:vertical;min-height:48px;max-height:200px;padding:.5rem .75rem;border:1px solid #d4c9b6;border-radius:8px;font-family:inherit;font-size:.95rem}
		.mnu-msg-form button{background:#245f4b;color:#fff;border:0;border-radius:8px;padding:0 1.25rem;font-weight:600;cursor:pointer}
		.mnu-msg-form button[disabled]{opacity:.6;cursor:wait}
...
CSS;
	...
	private static function inline_js(): string {
		...
		if (shop) {
			...
			api('sellers/' + sid).then(function(r){
				...
				if (TNMFrontend.currentUid && TNMFrontend.currentUid !== s.id) {
					...
					msg.className = 'tnm-button';
					msg.style.background = '#f4ecdc'; msg.style.color = '#245f4b';
					msg.href = '/messages/?to=' + s.id;
					msg.textContent = 'Message shop';
		...
		if (msg) {
			...
			var emptyEl = msg.querySelector('[data-mnu-thread-empty]');
			...
			function renderConversations(items) {
				if (!items.length && !preselect) { listEl.innerHTML = '<p class="mnu-muted">No messages yet. Open a shop to start a conversation.</p>'; return; }
				listEl.innerHTML = '';
				items.forEach(function(c){
					...
					row.innerHTML =
						...
						'<div class="mnu-conv-row__body">' +
							'<div class="mnu-conv-row__name">' + esc(c.user.store_name || c.user.display_name) + (c.unread ? '<span class="mnu-pill">New</span>' : '') + '</div>' +
							'<div class="mnu-conv-row__preview">' + esc(c.last_message) + '</div>' +
						'</div>';
					row.addEventListener('click', function(){ openThread(c.user.id, c.user); });
			...
			function openThread(uid, userMeta) {
				...
				Array.prototype.forEach.call(listEl.querySelectorAll('.mnu-conv-row'), function(r){
					r.classList.toggle('is-active', parseInt(r.getAttribute('data-user-id'),10) === uid);
				});
				emptyEl && (emptyEl.style.display = 'none');
				threadEl.innerHTML = '<div class="mnu-muted">Loading conversation…</div>';
				// If we don't have userMeta (fresh conv), fetch it.
				...
				Promise.all([ metaPromise, api('messages/' + uid) ]).then(function(res){
					...
					threadEl.innerHTML =
						'<div class="mnu-thread__head">' +
							'<img src="' + esc(user.avatar) + '" alt="" />' +
							'<div><strong>' + esc(user.store_name || 'Shop') + '</strong></div>' +
						'</div>' +
						'<div class="mnu-thread__msgs" data-mnu-thread-msgs></div>' +
						'<form class="mnu-msg-form" data-mnu-msg-form>' +
							'<textarea placeholder="Write a message…" data-mnu-msg-body required></textarea>' +
					...
					form.addEventListener('submit', function(ev){
						...
						api('messages', { method: 'POST', body: body }).then(function(r){
							...
							if (r.ok) {
								var el = document.createElement('div');
								el.className = 'mnu-msg mine';
								el.textContent = text;
								msgsEl.appendChild(el);
								msgsEl.scrollTop = msgsEl.scrollHeight;
								ta.value = '';
								productCtx = 0; // only attach product on the first message
							} else {
								alert((r.data && r.data.message) || 'Could not send message.');
							}
						});
					});
					ta.focus();

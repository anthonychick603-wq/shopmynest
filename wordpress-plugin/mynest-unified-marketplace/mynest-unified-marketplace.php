...
function mnu_load_files(): void {
	...
	$files = array(
		...
		'includes/class-mnu-admin-seller-diag.php',
		...
		'includes/class-mnu-shippo-tracking.php',
		...
		// v3.13.32 — email/phone/address gate for buyers at purchase and
		// sellers at listing. Requires ship-from-guard above for the seller
		// side to piggy-back on the product-publish filter chain.
		'includes/class-mnu-contact-guard.php',
		'includes/class-mnu-email-verify.php',
		'includes/class-mnu-signup.php',
		...
		'includes/class-mnu-bank-account.php',
		// v3.9.0 (Phase 3) — WP Admin → Marketplace → Payouts console.
		'includes/class-mnu-payouts-admin.php',
		// v3.13.30 Fix #17 — admin seller-debit endpoint with reason codes.
		'includes/class-mnu-seller-debit.php',
		// v3.13.21 — WP Admin → Marketplace → Reconciliation view.
		'includes/class-mnu-reconciliation-admin.php',
		'includes/class-mnu-app-links.php',
		'includes/class-mnu-favorites-listener.php',
		'includes/class-mnu-price-drop.php',
...
final class MNU_Plugin {
	...
	public function boot(): void {
		...
		// v3.13.16 — also compare plugin version, not just DB version. When
		// a release only changes code + settings migrations without touching
		// schema (MNU_DB_VERSION stays put), the previous DB-only guard
		// skipped MNU_Install::activate() and any set_defaults() migration
		// blocks silently didn't run for that release. Any change to either
		// version now triggers the upgrade path once.
		$installed_db  = (string) get_option( 'mnu_db_version', '' );
		$installed_ver = (string) get_option( 'mnu_version', '' );
		if ( MNU_DB_VERSION !== $installed_db || MNU_VERSION !== $installed_ver ) {
			MNU_Install::activate();
			$settings = (array) get_option( 'tnm_settings', array() );
		...
		// v3.7.122.1 — the auto-dedupe hook was pulled to isolate the activation
		// fatal. maybe_dedupe_pages() is still defined on the class and can be
		// invoked manually (e.g. via WP-CLI or a temporary must-use script) if
		// duplicates need cleaning. Manual bulk-trash from wp-admin is the
